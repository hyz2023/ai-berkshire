...
Generated from `skills/investment-team.md`. Preserve its research questions; apply
...
向用户展示以下团队结构；用户已明确请求研究时直接继续，无需重复确认：
...
### 第一步¾：联网能力预检（避免 Agent 静默退化）

**客户端分流**：Codex 使用其可用的搜索/读取工具实际执行一次只读查询，检查是否成功；不要读取 Claude 配置文件。以下白名单检查仅适用于 Claude Code。无论客户端，工具失败都必须明确说明，不能推断或伪装为联网成功。
